...
// add your code here
const a = document.querySelector('#pictures')
const z = document.querySelector('dialog')
const y = z.querySelector('img')
const x = z.querySelector('span')
...
for (let i = 0; i < 10; i++) {
    ...
    imgsource.src = picArray[i]['image']['medium']
    // imgsource.src = picArray[i]['image']['large']
    imgsource.alt = picArray[i]['title']

    const fig = document.createElement('figcaption')
    fig.innerText = picArray[i]['caption']

    const p = document.createElement('p')
    p.innerText = picArray[i]['description']
    b.appendChild(f)
    f.appendChild(imgsource)
    f.appendChild(fig)
    b.appendChild(p)
    b.addEventListener('click',()=>{
        y.src=picArray[i]['image']['large']
        y.alt=picArray[i]['title']
        z.showModal();
    })
